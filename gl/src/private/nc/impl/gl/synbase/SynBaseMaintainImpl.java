...
import nc.itf.gl.synbase.ISynBaseMaintain;
...
public class SynBaseMaintainImpl implements ISynBaseMaintain {

	@SuppressWarnings("unchecked")
	@Override
	public void synBasedoc(String oldts, String newts, int type,TempBaseDocVO tempvo) throws BusinessException {
		
		BaseDAO daoA=new BaseDAO(BaseDocType.design_A);
		BaseDAO daoB=new BaseDAO(BaseDocType.design_B);
		
		String tableA=BaseDocType.accountA;
		String tableB=BaseDocType.accountB;
...
//=========================================================同步部门表(def20做为关联键)==================================
				...
				//==================先查询部门涉及的业务单元====================================================
				String sqlunit="select distinct g.* from org_dept s left join v_tcl_org g on g.pk_org=s.pk_org "
				...
				//===========================查询编码重复问题(业务单元维度)====================================
				String exsql_dept="select distinct g.code||'-'||s.code from org_dept s left join v_tcl_org g on s.pk_org=g.pk_org "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"' "
...
//=========================================================同步人员表(def20做为关联键)==================================
				...
				//==================先查询人员涉及的业务单元====================================================
				String sqlunit="select distinct g.* from bd_psndoc s left join v_tcl_org g on g.pk_org=s.pk_org "
						+ "where s.ts>='"+oldts+"' and s.ts<='"+newts+"' ";
				List<Map<String, Object>> mapunit=(List<Map<String, Object>>)daoA.executeQuery(sqlunit, new MapListProcessor());
...
//=================================================同步表psnjob=================================================
				...
				//==================先查询人员涉及的业务单元====================================================
				String sqljob="select distinct g.* from bd_psnjob s left join v_tcl_org g on g.pk_org=s.pk_org "
						+ "where s.ts>='"+oldts+"' and s.ts<='"+newts+"' ";
				...
				//==================先查询人员涉及的部门====================================================
				String sqldept="select distinct g.*,sss.code orgcode from bd_psnjob s left join v_tcl_dept g on g.pk_dept=s.pk_dept "
						+ " left join v_tcl_org sss on s.pk_org=sss.pk_org where s.ts>='"+oldts+"' and s.ts<='"+newts+"' ";
...
//=========================================================同步客商表(def20做为关联键)不同步分类以及银行 账户==================================
				...
				//==================先查询客户涉及的业务单元====================================================
				String sqlunit="select distinct g.* from v_tcl_org g ";
				List<Map<String, Object>> mapunit=(List<Map<String, Object>>)daoA.executeQuery(sqlunit, new MapListProcessor());
...
//==================================================同步表bd_customer=================================================
				...
				for(CustFinanceVO vo:cflist){
					...
					String sqlm="select r.pk_customer,r.code,rr.pk_customer pk_cust,g.pk_org,g.pk_org2 "
							...
							+ " left join v_tcl_org g on r.pk_org=g.pk_org where r.pk_customer='"+pk_cust+"' ";
...
//=========================================================同步物料分类表(def2做为关联键)===============================================
				...
				//===========================查询编码重复问题(业务单元)====================================
				String exsql_dept="select distinct g.code||'-'||s.code from bd_marbasclass s left join v_tcl_org g on s.pk_org=g.pk_org "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"' "
...
//============================================================物料信息bd_material======================================================================
				...
				//==================先查询物料涉及的分类====================================================
				String sqldept="select distinct g.*,sss.code orgcode from bd_material s left join v_tcl_marbasclass g on g.pk_marbasclass=s.pk_marbasclass "
						+ " left join v_tcl_org sss on s.pk_org=sss.pk_org where s.ts>='"+oldts+"' and s.ts<='"+newts+"'  ";
				...
				for(MaterialFiVO vo:cflist2){
					...
					String sqlm="select r.pk_material,r.code,rr.pk_material pk_cust,g.pk_org,g.pk_org2 "
							+ " from bd_material r left join "+tableB+".bd_material rr on r.pk_material=rr.def20 "
							+ " left join v_tcl_org g on r.pk_org=g.pk_org where r.pk_material='"+pk_cust+"' ";
					Map<String, Object> map=(Map<String, Object>)daoA.executeQuery(sqlm, new MapProcessor());
					if(map==null){
						continue ;
					}
...
//============================================================银行档案bd_bankdoc======================================================================
				...
				//==================先查询档案涉及的类别====================================================
				String sqldept="select distinct g.* from bd_bankdoc s left join v_tcl_banktype g on g.pk_banktype=s.pk_banktype "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"' ";
				List<Map<String, Object>> listclass=(List<Map<String, Object>>)daoA.executeQuery(sqldept, new MapListProcessor());
...
//==============================================同步表银行账户===============================================================================
				...
				//==================先查询账户涉及的类别====================================================
				String sqldept3="select distinct g.* from bd_bankaccbas s left join v_tcl_banktype g on g.pk_banktype=s.pk_banktype "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"'  ";
				...
				//==================先查询账户涉及的银行档案====================================================
				String sqldept4="select distinct g.* from bd_bankaccbas s left join v_tcl_bankdoc g on g.pk_bankdoc=s.pk_bankdoc "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"'  ";
...
//====================================================同步开始================================================================
				...
				for(BankAccUseVO vo:cflist2){
					...
					String sqlm="select r.pk_bankaccbas,r.accnum,rr.pk_bankaccbas pk_mate,g.pk_org,g.pk_org2 "
							+ " from bd_bankaccbas r left join "+tableB+".bd_bankaccbas rr on r.pk_bankaccbas=rr.def2 "
							+ " left join v_tcl_org g on r.pk_org=g.pk_org where r.pk_bankaccbas='"+pk_bank+"' ";
					Map<String, Object> map=(Map<String, Object>)daoA.executeQuery(sqlm, new MapProcessor());
					if(map==null){
						continue ;
					}
...
//==================================================同步表bd_account====================================================
				//==================先查询涉及的科目表====================================================
				String sqlunit="select distinct g.* from v_tcl_accchart g ";
				List<Map<String, Object>> mapunit=(List<Map<String, Object>>)daoA.executeQuery(sqlunit, new MapListProcessor());
...
//==========================================================================================同步表bd_accasoa=================================================
				//查询涉及的科目
				String sqldept="select distinct g.* from bd_accasoa s left join v_tcl_account g on g.pk_account=s.pk_account "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"' ";
				List<Map<String, Object>> listclass=(List<Map<String, Object>>)daoA.executeQuery(sqldept, new MapListProcessor());
...
//==========================================================================================同步表bd_accass=================================================
			...
			try {
				...
				//==================先查询档案涉及的类别====================================================
				String sqldept="select distinct g.* from bd_defdoc s left join v_tcl_list g on g.pk_defdoclist=s.pk_defdoclist "
						+ " where s.ts>='"+oldts+"' and s.ts<='"+newts+"' and exists(select 1 from v_tcl_list v where v.pk_defdoclist=s.pk_defdoclist) ";
				List<Map<String, Object>> listclass=(List<Map<String, Object>>)daoA.executeQuery(sqldept, new MapListProcessor());
				...
				String sqldept2="select * from bd_defdoc s where s.ts>='"+oldts+"' and s.ts<='"+newts+"' "
						+ " and exists(select 1 from v_tcl_list v where v.pk_defdoclist=s.pk_defdoclist) ";
