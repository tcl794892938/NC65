...
public class DataBackImportUI extends ToftPanel implements ActionListener {
	...
	private String setDefaultData(){
		...
		String str="";
		String sql="select * from temp_ts where temp_id='tcl' ";
		IUAPQueryBS iQ=NCLocator.getInstance().lookup(IUAPQueryBS.class);
		
		try {
			vo=(TempBaseDocVO)iQ.executeQuery(sql, new BeanProcessor(TempBaseDocVO.class));
	 ...
	 * 接口
	 */
	private ISynBaseMaintain getService(){
		if(isbm==null){
			isbm=NCLocator.getInstance().lookup(ISynBaseMaintain.class);
		}
		return isbm;
	}
	...
	private ISynBaseMaintain isbm=null;
	private TempBaseDocVO vo=null;
