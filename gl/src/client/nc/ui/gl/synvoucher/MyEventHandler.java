...
import nc.vo.fl.voucher.BillVoucherHVO;
...
public class MyEventHandler extends AbstractMyEventHandler {
	...
	public class MyThread extends Thread {
		...
		public void run() {
			try {
				...
				// 同步完写入同步信息,刷新表体
				BillVoucherHVO hvo = (BillVoucherHVO) vo.getParentVO();
				...
				rdlg.showRizhiInfomation(yearMonth, bpanel, ccorp,FileAbsPath.SENDPATH1);
	...
	protected void onBoSave() throws Exception {
		...
		String sql = "select yearmonth from fl_voucher_h where nvl(dr,0)=0 "
	...
	private void outXmlByYearMonthAndCorp(String yearMonth, String pk_corp,
		...
		for (int i = 0; i < vos.length; i++) {
			...
			if (vlist.size() == 800) {
				...
				try {
					returnstr+=xmltools.outputXMLDoc(sendVos, ccorp,url,FileAbsPath.send_account);
				} catch (Exception e) {
				}
				vlist.clear();
			}
		...
		try {
			...
			FileOutputStream output=new FileOutputStream(FileAbsPath.SENDPATH1+filename);
	...
	protected void onBoDelete() throws Exception {
		...
		HYPubBO_Client.deleteByWhereClause(BillVoucherHVO.class, " pk_voucher_h='"+objpk+"'");
